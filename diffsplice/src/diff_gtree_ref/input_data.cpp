...
void input_junction_merged(string chromosome, string filename_junc_merged)
{
	...
	while (juncfile >> occurrence)
	{
...
#else
		new_junction = new fragment (SUPPORT_VECTOR_SIZE_WHOLE_DATA);
#endif
		...
		if (!junction_array.empty() && position_start == junction_array.back()->start && position_end == junction_array.back()->end) {
...
#else
			if (occurrence > junction_array.back()->support[SUPPORT_VECTOR_SIZE_WHOLE_DATA]) {
				junction_array.back()->transDirection = field_xs == 0 ? antisense : sense;
			}
			for (sample_cnt = 0; sample_cnt < SUPPORT_VECTOR_SIZE_WHOLE_DATA; ++sample_cnt) {
				juncfile >> tmp_val;
				junction_array.back()->support[sample_cnt] += tmp_val;
			}
			junction_array.back()->support[SUPPORT_VECTOR_SIZE_WHOLE_DATA] += occurrence;
#endif
			...
			for (sample_cnt = 0; sample_cnt < SUPPORT_VECTOR_SIZE_WHOLE_DATA; ++sample_cnt)
				juncfile >> new_junction->support[sample_cnt];
			new_junction->support[SUPPORT_VECTOR_SIZE_WHOLE_DATA] = occurrence;
#endif
...
void label_select_junctions(string filename_annotation)
{
	...
	if (!filename_annotation.empty()) {
		...
		if (infile.is_open())
		{
			...
			while (infile >> chrname)
			{
				...
				infile >> strand;
				getline(infile, info);
				trans_direction strand_anno = strand.compare("-") == 0 ? antisense : sense;
				...
				if (junction_array[junctionLoop]->start == start_pos && junction_array[junctionLoop]->end == end_pos)
				{
					...
					if (junction_array[junctionLoop]->transDirection != strand_anno)
					{
						junction_array[junctionLoop]->transDirection = strand_anno;
						outfile_debuginfo << "override strand for " << chrname << ":" << start_pos << "-" << end_pos << " to " << strand << endl;
					}
					++junctionLoop;
	...
	for (junctionLoop = 0; junctionLoop < junction_array.size(); ++junctionLoop)
	{
		...
		if (pass_filter || (use_white_list && junction_array[junctionLoop]->in_junc_annotation))
		{
			...
			tmp_support[SUPPORT_VECTOR_SIZE] = junction_array[junctionLoop]->support[SUPPORT_VECTOR_SIZE_WHOLE_DATA];
...
void extract_exons(RangeJunctionList *cur_range, string chromosome)
{
	...
	new_feature->transDirection = curSite->strand;
	...
	for (tmpLoop = 1; tmpLoop < splicesite_list_cnt; ++tmpLoop)
	{
		...
		else
		{
			...
			if (new_feature->end >= new_feature->start && (new_feature->type == frag_exon || new_feature->exon_strand_start == new_feature->exon_strand_end))
			{
				// determine the strand of the exon
				if (new_feature->exon_strand_start == new_feature->exon_strand_end) {
					new_feature->transDirection = new_feature->exon_strand_start;
				}
				else {
					if (new_feature->altersite == 0)
					{
						new_feature->transDirection = mixed;
					}
					else if (new_feature->altersite == 1)
					{
						new_feature->transDirection = new_feature->exon_strand_end;
					}
					else if (new_feature->altersite == 2)
					{
						new_feature->transDirection = new_feature->exon_strand_start;
					}
				}
...
double cutAlterSite(long startIndex, long endIndex, int altersitetype, long long &totalSupport, long long &cutFragSupport, long &cutFragStart, long &cutFragEnd)
{
	double maxRatio = 0, curRatio;
	long long supp_left, supp_right;
	...
	if (altersitetype == 1)
	{
		for (size_left = 0, supp_left = 0; size_left < vectorLength - 1; ++size_left)
		...
		for (curPos = vectorLength - 1; curPos > 0; --curPos)
		{
			if (supp_left < 1)
			{
				...
				curRatio = (double(supp_right) / size_right) / (double(supp_left) / size_left);
	...
	else if (altersitetype == 2)
	{
		...
		for (size_right = 0, supp_right = 0; size_right < vectorLength; ++size_right)
		...
		for (curPos = 0; curPos < vectorLength; ++curPos)
		{
			if (supp_right < 1)
			{
				...
				curRatio = (double(supp_left) / size_left) / (double(supp_right) / size_right);
...
void trim_exons(RangeJunctionList *cur_range, int index)
{
	...
	long startPosition, tmp, tmpVectorLength, iLoop, curFragStart, tmpExonAddLength, altersiteFragStart, altersiteFragEnd, max_nocoverage_length_in_use = MAX_NOCOVERAGE_LENGTH;
	long long curSupport, altersiteFragSupport;
	...
	while (cur_feature != NULL)
	{
		...
		if (cur_feature->junc->type == frag_exon && cur_feature->junc->altersite > 0 && tmpVectorLength <= MAX_ALTER_SPLICE_SITE_LENGTH) // 10/13/2013 add a maximum length, longer exons should go to the exon cut. important in real data, e.g. chr1:113946277-114059752 in brca
		{
			...
			if (changeRatio < COVERAGE_CHANGE_THRESH_ALTER_SITE)
			{
				...
				if (tmpExonAddLength >= MIN_ALTER_SPLICE_SITE_LENGTH && (double(curSupport) / SUPPORT_VECTOR_SIZE_WHOLE_DATA) >= (coverageThreshold_exon * tmpExonAddLength))
				{
					...
					new_fragment->transDirection = cur_feature->junc->transDirection;
			...
			else
			{
				...
				if (tmpExonAddLength >= MIN_EXON_LENGTH && (double(altersiteFragSupport) / SUPPORT_VECTOR_SIZE_WHOLE_DATA) >= (coverageThreshold_exon * tmpExonAddLength))
				{
					//get an end, make a new fragment
					new_fragment = new fragment(SUPPORT_VECTOR_SIZE);
					new_fragment->type = cur_feature->junc->type; //frag_exon;
			...
			for (iLoop = 0; iLoop < tmpVectorLength; ++iLoop)
			{
				...
				if (((chromosome_expr[cur_feature->junc->start + iLoop] <= thresh_exon_cov_in_use * SUPPORT_VECTOR_SIZE_WHOLE_DATA || changeRatio < 1/COVERAGE_CHANGE_THRESH_EXON) && largeGap == true) || iLoop == tmpVectorLength - 1)
				{
					if (makingNewFrag == true)
					{
						...
						if (tmpExonAddLength >= MIN_EXON_LENGTH && (double(curSupport) / SUPPORT_VECTOR_SIZE_WHOLE_DATA) >= (thresh_exon_cov_in_use * tmpExonAddLength))
						{
							//get an end, make a new fragment
							new_fragment = new fragment(SUPPORT_VECTOR_SIZE);
							...
							if (new_fragment->exon_strand_start == terminal && new_fragment->exon_strand_end == terminal)
							{
								new_fragment->transDirection = undetermined;
							}
							else if (new_fragment->exon_strand_start == terminal)
							{
								new_fragment->transDirection = new_fragment->exon_strand_end;
							}
							else if (new_fragment->exon_strand_end == terminal)
							{
								new_fragment->transDirection = new_fragment->exon_strand_start;
							}
							else {
								new_fragment->transDirection = (new_fragment->exon_strand_start == new_fragment->exon_strand_end) ? new_fragment->exon_strand_start : mixed;
							}
		...
		else if (cur_feature->junc->type == frag_retained_intron)
		{
			for (iLoop = 0; iLoop < tmpVectorLength; ++iLoop)
			{
				...
				if (((changeRatio < 1/COVERAGE_CHANGE_THRESH_EXON || chromosome_expr[cur_feature->junc->start + iLoop] <= coverageThreshold_intron * SUPPORT_VECTOR_SIZE_WHOLE_DATA) && largeGap == true) || iLoop == tmpVectorLength - 1)
				{
					...
					{
						tmpExonAddLength = cur_feature->junc->start + iLoop - curFragStart + 1;
						if (tmpExonAddLength >= MIN_EXON_LENGTH && (double(curSupport) / SUPPORT_VECTOR_SIZE_WHOLE_DATA) >= (coverageThreshold_intron * tmpExonAddLength)) //&& tmpExonAddLength < tmpVectorLength - 1
						{
							//get an end, make a new fragment
							new_fragment = new fragment(SUPPORT_VECTOR_SIZE);
							new_fragment->type = cur_feature->junc->type;
		...
		if (exon_code > 0)
		{
			...
			if (refine_exon_bound(exon_coverage, num_cut_pt, exon_code, cut_pt, avg_coverage))
			{
				...
				{		
					...
					new_fragment->exon_strand_start = terminal;
					new_fragment->exon_strand_end = cur_feature->junc->exon_strand_end;
					new_fragment->transDirection = cur_feature->junc->exon_strand_end;
					
					cur_feature->junc->end = cur_feature->junc->start - 1 + cut_pt[0];
					cur_feature->junc->support[SUPPORT_VECTOR_SIZE_WHOLE_DATA] = avg_coverage[0];
					cur_feature->junc->exon_strand_end = terminal;
					cur_feature->junc->transDirection = cur_feature->junc->exon_strand_start;
...
void input_data(RangeJunctionList *list_fragments, string dir_input_frag, string dir_junction_annotation, long chromosome_length)
{
	...
	cur_feature = list_fragments->list;
	while (cur_feature)
	{
		for (int sampleLoopCnt = 0; sampleLoopCnt < SUPPORT_VECTOR_SIZE; ++sampleLoopCnt)
		{
			cur_feature->junc->support[sampleLoopCnt] = cur_feature->junc->support[sampleLoopCnt] * DatasetNormalizationRatio[sampleLoopCnt];
		}
		cur_feature = cur_feature->next;
	}
#endif
...
void collect_expr_ref_exons(vector<fragment *> *ref_exons) {
	...
	for (auto it_frag = ref_exons->begin(); it_frag != ref_exons->end(); ++it_frag) {
		...
		(*it_frag)->support[0] = double(expr) / double((*it_frag)->end - (*it_frag)->start + 1) / double(SUPPORT_VECTOR_SIZE_WHOLE_DATA);
	}
}
...
void extract_exons_ref(string chromosome, const vector<fragment *> &ref_exons, RangeJunctionList *cur_range) 
{
	...
	if (splicesite_vec.front().position > cur_range->rangeLow) { // coverage before the first annotated exon
		...
		new_feature->transDirection = splicesite_vec.front().strand;
	...
	if (splicesite_vec.back().position < cur_range->rangeHigh) { // coverage before the first annotated exon
		...
		new_feature->exon_strand_end = terminal;
		new_feature->transDirection = splicesite_vec.back().strand;
...
// 		while (cur_feature != NULL)
// 		{
// 			if (cur_feature->junc->start == 117078686 && cur_feature->junc->end == 117078780) {
// 				cout << 'a';
// 			}
// 			cur_feature = cur_feature->next;
// 		}
