...
ofstream outfile_asm; // output ASM information for Joshua
...
string tostr_ASMcategory(const int code) {
	string str_category;
	switch (code)
	{
	case unknown:
		str_category = "combination";
		break;
	case exon_skipping:
		str_category = "exon_skipping";
		break;
	case mutual_exclusive:
		str_category = "mutual_exclusive";
		break;
	case intron_retention:
		str_category = "intron_retention";
		break;
	case alter_splice_site:
		str_category = "alter_splice_site";
		break;
	case alter_start:
		str_category = "alter_start/end";
		break;
	case alter_end:
		str_category = "alter_start/end";
		break;
	case diff_expression:
		str_category = "diff_expression";
		break;
	case nested:
		str_category = "nested";
		break;
...
void extract_exons(alternative_path *asm_path, vector<pair <long, long> > &exonList, bool &nested)
{
	nested = false;
	if (!asm_path){
		exonList.clear();
	...
	else if (asm_path->pathVertex->childType == 1 || asm_path->pathVertex->childType == 2)
	{
		...
		while (path_component != NULL)
		{
			...
			if (cur_component->childNum > 1) {
				nested = true; // this child is a composite ASM
			}
...
bool overlapping_path(alternative_path *path_A, alternative_path *path_B)
{
	...
	extract_exons(path_A, exonList_A, nested_A);
	if (exonList_A.empty()){
		return false;
	}
	extract_exons(path_B, exonList_B, nested_B);
	if (exonList_B.empty()){
		return false;
	}
...
bool GTree_output(GTvertex *rootVertex, int sign, ofstream *outputfile)
{
	...
	(*outputfile) << tostr_ASMcategory(vertexCategory) << " ";
	...
	if (rootVertex->child == NULL)
	{
		if (OUTPUT_PER_SAMPLE_EXPR) {
			...
			(*outputfile) << "{" << calculate_mean(rootVertex->support, SUPPORT_VECTOR_SIZE)  << "/" << calculate_mean(rootVertex->proportion, SUPPORT_VECTOR_SIZE) << "}";
		}
		...
		while (curJunc != NULL)
		{
			...
			if (OUTPUT_PER_SAMPLE_EXPR) {
				...
				(*outputfile) << calculate_mean(curJunc->junc->support, SUPPORT_VECTOR_SIZE) << "";
			}
			(*outputfile) << ") ";
...
// 	}
	...
	GTvertex *prevVertex = targetVertex->find_prevvtx();
	GTvertex *nextVertex = targetVertex->find_nextvtx();
	...
	while (asm_path != NULL)
	{
		...
		outfile_asm << chromosomeName << "." << targetVertex->vertex_id << "\t" << chromosomeName << "." << asm_path->pathVertex->vertex_id << "\t" << chromosomeName << ":" << asm_path->path_start << "-" << asm_path->path_end << ":" << strand << "\t" << tostr_ASMcategory(targetVertex->ASMcategory);
		for (int sampleLoopCnt = 0; sampleLoopCnt < SUPPORT_VECTOR_SIZE; ++sampleLoopCnt)
			outfile_asm << "\t" << asm_path->support[sampleLoopCnt];

		if (prevVertex && prevVertex->exonNum > 0 && abs(asm_path->path_start - prevVertex->rangeHigh) < 2) {
			outfile_gtf_asm_path_decomposed << chromosomeName << "\t" << "ASM" << "\t" << "exon" << "\t" << prevVertex->rangeLow << "\t" << prevVertex->rangeHigh << "\t" << "." << "\t" << strand << "\t" << "." << "\t"
			...
			outfile_asm << "\t" << prevVertex->rangeLow << "-" << prevVertex->rangeHigh;
		}
			...
			while (curJunc != NULL)
			{
				if (curJunc->junc->type == frag_exon || curJunc->junc->type == frag_retained_intron) {
					outfile_gtf_asm_path_decomposed << chromosomeName << "\t" << "ASM" << "\t" << "exon" << "\t" << curJunc->junc->start << "\t" << curJunc->junc->end << "\t" << "." << "\t" << strand << "\t" << "." << "\t" 
						<< "gene_id \"" << chromosomeName << "." << targetVertex->vertex_id + name_suffix1 << "\"; " << "transcript_id \"" << chromosomeName << "." << asm_path->pathVertex->vertex_id + name_suffix2 << "\";" << endl;
					outfile_asm << "\t" << curJunc->junc->start << "-" << curJunc->junc->end;
				}
			...
			while (path_component != NULL)
			{
				...
				if (cur_component->exonNum > 0) {
					outfile_gtf_asm_path_decomposed << chromosomeName << "\t" << "ASM" << "\t" << "exon" << "\t" << cur_component->rangeLow << "\t" << cur_component->rangeHigh << "\t" << "." << "\t" << strand << "\t" << "." << "\t"
						<< "gene_id \"" << chromosomeName << "." << targetVertex->vertex_id + name_suffix1 << "\"; " << "transcript_id \"" << chromosomeName << "." << asm_path->pathVertex->vertex_id + name_suffix2 << "\";" << endl;
					outfile_asm << "\t" << cur_component->rangeLow << "-" << cur_component->rangeHigh;
				}
		...
		if (nextVertex && nextVertex->exonNum > 0 && abs(asm_path->path_end - nextVertex->rangeLow) < 2) {
			outfile_gtf_asm_path_decomposed << chromosomeName << "\t" << "ASM" << "\t" << "exon" << "\t" << nextVertex->rangeLow << "\t" << nextVertex->rangeHigh << "\t" << "." << "\t" << strand << "\t" << "." << "\t"
				<< "gene_id \"" << chromosomeName << "." << targetVertex->vertex_id + name_suffix1 << "\"; " << "transcript_id \"" << chromosomeName << "." << asm_path->pathVertex->vertex_id + name_suffix2 << "\";" << endl;
			outfile_asm << "\t" << nextVertex->rangeLow << "-" << nextVertex->rangeHigh;
		}
		outfile_asm << endl;
...
void output_ASM_composition(GTvertex *targetVertex)
{
	...
	outfile_asm_composition << targetVertex->vertex_id << "\t" << chromosomeName << "\t" << targetVertex->rangeLow << "\t" << targetVertex->rangeHigh << "\t" << tostr_ASMcategory(targetVertex->ASMcategory) << "\t" << targetVertex->childNum << "\t";
...
{	
	...
	filename = resultPath + "asm/" + chromosomeName + "_asm.txt";
	...
	filename = resultPath + "stat/splice_all.bed";
	outfile_junction_all.open(filename.c_str());
	filename = resultPath + "stat/splice_filtered.bed";
	outfile_junction_filtered.open(filename.c_str());
