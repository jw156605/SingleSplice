...
GTvertex* GTvertex::find_prevvtx() 
{
	if (prevSibling) {
		return prevSibling;
	}
	GTvertex *ancestor_vtx = parent;
	while (ancestor_vtx)
	{
		if (ancestor_vtx->prevSibling)
		{
			return ancestor_vtx->prevSibling;
		}
		ancestor_vtx = ancestor_vtx->parent;
	}
	return NULL;
}

GTvertex* GTvertex::find_nextvtx() 
{
	if (nextSibling) {
		return nextSibling;
	}
	...
	while (ancestor_vtx)
	{
		if (ancestor_vtx->nextSibling)
		{
			return ancestor_vtx->nextSibling;
		}
		ancestor_vtx = ancestor_vtx->parent;
	}
	return NULL;
