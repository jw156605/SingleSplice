...
bool extensible_direction(trans_direction dir_low, trans_direction dir_high)
{
	return dir_low == undetermined || dir_high == undetermined || dir_low == dir_high || dir_low == mixed || dir_high == mixed;
}
	...
	while (curVertex != NULL)
	{
		if (curVertex->vertexType == normal)
		{
			...
			while (edgeVertex != NULL)
			{
				...
				{
					if (compatibleJunctions((curVertex->corresJunc), (edgeVertex->corresJunc)))
					{
...
void constructJuncGraph_directed(RangeJunctionList* origList)
{
	...
	while (curVertex != NULL)
	{
		...
		while (edgeVertex != NULL)
		{
			if (compatibleJunctions(curVertex->corresJunc, edgeVertex->corresJunc) && extensible_direction(curVertex->corresJunc->junc->transDirection, edgeVertex->corresJunc->junc->transDirection))
			{
				//create an edge for curVertex
				newEdge = new JuncGraphEdge;
				newEdge->linkedVertex = edgeVertex;
...
RangeJunctionList* separateIndepPath(RangeJunctionList* origList, bool &separable)
{
	...
	RangeJunctionList *curList = resultList;
	while (curList)
	{
		curList->transDirection = curList->list->junc->transDirection; // have not resolved the mixed strand case
...
{	
	...
	while (curVertex != NULL)
	{
		...
		if (curVertex->hasInEdge == false || curVertex->corresJunc->junc->transDirection == mixed) // 3/27/2015 add the 2nd condition to resolve overlapped genes with different strands, otherwise the 2nd gene often gets discarded because the path cannot be extended at the shared exon due to incompatible strand
		{
			...
			newList->transDirection = curVertex->corresJunc->junc->transDirection == mixed ? undetermined : curVertex->corresJunc->junc->transDirection;
		...
		if (curPath->arrivedVertex->edges != NULL)
		{
			...
			while (curEdge != NULL)
			{
				if (curEdge->linkedVertex->corresJunc->junc->end >= curPath->arrivedVertex->corresJunc->junc->end)
				{
					...
					//	|| curEdge->linkedVertex->corresJunc->junc->transDirection == mixed)
					if (extensible_direction(curPath->pathJuncList->transDirection, curEdge->linkedVertex->corresJunc->junc->transDirection))
					{
						pathExtended = true;
...
GTvertex* constructGTree(RangeJunctionList* origList)
{
	...
	while (curVertex != NULL)
	{
// 		if (curVertex->rangeLow <= 117079613 && curVertex->rangeHigh >= 117079769)
// 		{
...
// 			}
			...
			if (separable_depPath == true && num_enumerated_path <= MAX_NUM_PATH_FOR_NO_DECOMPOSITION)
			{
				...
				while (curList != NULL)
				{
					...
					newVertex->transDirection = curList->transDirection;
