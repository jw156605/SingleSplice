...
void difftrans_test_multi_class(string resultPath, string resultFileSuffix)
{
	...
	cout << resultFileSuffix << ":\t#gene = " << sortList_ASM.size()-1 << "\t#permutation = " << MAX_PERMUTATION_NUMBER << "\t#allpermutedvalue = " << allpermutedvalueCnt << endl;
	...
	FDRfile << "shifting delta will call different sets of significant loci and will result in different FDRs" << endl;
	...
	for (delta = 0; delta <= 10; delta += 0.01)
	{
		...
		FDRfile << "delta = " << delta << ": " << significanceCnt << " loci picked, with FDR(median) = "
			<< FDR_median << " and FDR(mean) = " << FDR_mean << " and FDR(90percentile) = " << FDR_90percentile << endl;

		//cout << "delta = " << delta << ": " << significanceCnt << " loci picked, with FDR(median) = "
		//	<< FDR_median << " and FDR(mean) = " << FDR_mean << " and FDR(90percentile) = " << FDR_90percentile << endl;
